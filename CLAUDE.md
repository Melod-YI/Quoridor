...
- Plan 4 验收收口(2026-07-06):PreviewService 补 Kid 变体覆盖 + 精确步数 + 忽略墙数 4 测试;`demo --acceptance` 半自动回归(换位断言 + Kid 终局流 + 预览合法非法);手动验收清单 `docs/superpowers/acceptance/2026-07-06-plan4-acceptance-checklist.md`(7 项,视觉部分需真人跑)

**测试**:`dotnet test Quoridor.slnx` → Domain 74 + Application 45 + UI.Logic 45 = 164 全绿。
...
验收项2-7(逻辑层已收口:4 测试 + `demo --acceptance`;手动视觉清单待勾)/ Plan 5 剩余(Setting 面板;Replay 已于 2026-07-06 完成) / Plan 6(Kid主题+4人+动画) / UI 美化(Container+Anchor 布局+真实 Theme,替代硬编码坐标)。
